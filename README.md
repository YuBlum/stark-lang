...
You can also create distinct type alises using the `type()` constructor: 
```
...
It's also possible to define multiple constants at once with separate them by a comma:
```
...
### Constant scoping
When a constant is defined it'll become avaiable to all the current scope and all scopes inside the current scope. So forward declaring is possible:
```
...
### Unused constants
Simmilarly to [unused arguments](#Unused-arguments), unused constants also throw compile errors:
```
...
| 0567             | Oct literal      | anyi (can be passed to any integer) | Inferred by first usage (u8, i16, etc). If needed defaults to i32 |
...
| 123i8            | i8 literal       | i8                                  | i8                                                                |
| 123i16           | i16 literal      | i16                                 | i16                                                               |
| 123i32           | i32 literal      | i32                                 | i32                                                               |
| 123i64           | i64 literal      | i64                                 | i64                                                               |
...
| 0x123abci8       | Hex i8 literal   | i8                                  | i8                                                                |
| 0x123abci16      | Hex i16 literal  | i16                                 | i16                                                               |
| 0x123abci32      | Hex i32 literal  | i32                                 | i32                                                               |
| 0x123abci64      | Hex i64 literal  | i64                                 | i64                                                               |
...
### The 'null' literal
`null` is the zero value for pointers. It is a pointer that point's to nothing.

If you create a pointer and not give it a value it'll default to `null`:
```
def ptr = *i32; # defaults to 'null'
```

### Void pointer
[The void type](#The-void-type) is a type that holds no value. So a pointer to a `void` is basically a pointer to nothing or anything:
```
def a = i32;
def b = f64;
def c = Some_Struct;
...
## Casting
Stark is strongly typed. You can't pass a u8 to an f32 or even an i8 to an i16. But sometimes casting is needed, on those cases use the `->` operator:
```
def a = 1.5;
...
It's not possible to cast a immutable pointer into a mutable one:
```
def a = i32;
def p0 = &a;
...
It's not possible to cast a integer into a mutable pointer:
```
def a = 0xdeadbeef;
def p1 = i0 -> *mut i32; # error
```

Sometimes you don't care about the underling type and just want to cast it automatically. Use the `!` operator for this:
```
def sum : (a, b = i32) i32 => a+b;
def x = sum(12, 1.6!); # x = 13
```
